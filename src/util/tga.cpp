...
// allocate an array for the new image data
	newImageData = new unsigned char [ sizeof(unsigned char) * tgaFile.height * tgaFile.width ];
	if (newImageData == NULL) {
		return;
	}

// convert pixels: greyscale = o.30 * R + 0.59 * G + 0.11 * B
	for (i = 0,j = 0; j < tgaFile.width * tgaFile.height; i +=mode, j++)
		newImageData[j] =	(unsigned char)(0.30 * tgaFile.data[i] + 
						0.59 * tgaFile.data[i+1] +
						0.11 * tgaFile.data[i+2]);
...
// reassign pixelDepth and type according to the new image type
	//tgaFile.pixelDepth = 8;
	tgaFile.type[2] = 3;
// reassing imageData to the new array.
	tgaFile.data = newImageData;
}
